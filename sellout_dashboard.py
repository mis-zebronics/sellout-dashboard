...
# PRIORITY-BASED column finder - looks for FY first, then YTD
def find_col_priority(df, search_text, exclude_words=None):
    exclude_words = exclude_words or []
    # First pass: exact "FY" match (highest priority)
...
# Find FY columns with PRIORITY on FY over YTD
...
C["ytd25_g"] = find_col_priority(df, "ytd 25-26 gms", ["plan","ach","growth"])
C["ytd26_g"] = find_col_priority(df, "ytd 26-27 gms", ["plan","ach","growth"])

st.sidebar.markdown("### Column Mapping")
for k, v in C.items():
    st.sidebar.write("**" + str(k) + "**: " + str(v if v else "NOT FOUND"))
...
for cx in ["LY GMS", "CY GMS Plan", "CY GMS Actual"]: qdf[cx] = qdf[cx].apply(lambda x: "Rs"+str(round(x/1e7,2))+"Cr" if abs(x) >= 1e7 else "Rs"+str(round(x/1e5,2))+"L")
